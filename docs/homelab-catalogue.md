...
0.1.3 is now saved, not just the product eventually selected. Phones can browse those saved names,
...
3. Press **Reload** and confirm version `0.1.3`.
...
If a product has no image, search for it again with extension 0.1.3. Older rows saved by extension
