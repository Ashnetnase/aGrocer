...
0.1.2 is now saved, not just the product eventually selected. Phones can browse those saved names,
...
3. Press **Reload** and confirm version `0.1.2`.
...
If a product has no image, search for it again with extension 0.1.2. Older rows saved by extension
